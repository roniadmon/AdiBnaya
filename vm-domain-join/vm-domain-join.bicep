...
resource virtualMachine 'Microsoft.Compute/virtualMachines@2021-03-01' = [for i in range(0, 2): {
  ...
  properties: {
    ...
    storageProfile: {
      ...
      osDisk: {
        name: '${dnsLabelPrefix}-OsDisk${i+1}'
        caching: 'ReadWrite'
        createOption: 'FromImage'
      }
      dataDisks: [
        {
          name: '${dnsLabelPrefix}-DataDisk${i+1}'
          caching: 'None'
          createOption: 'Empty'
          diskSizeGB: 1024
          lun: 0
...
resource virtualMachineExtension 'Microsoft.Compute/virtualMachines/extensions@2021-03-01' = [for i in range(0, 2): {
